...
result=function(l){
  ...
  for (i in seq(2:p)){
    ...
    pred = join(t_coef, pred, type = "full")
  }
  t_coef = as.data.frame(t(lasso1(p,l)))
  pred = join(t_coef, pred, type = "full")
  pred = pred[, -1]
...
cf_mat=function(actu,pred){
  ...
  repeat{
    ...
    repeat{
      if(col_fill == p+1)
        break
      if(pred[row_fill,col_fill] == actu[row_fill,col_fill])
      ...
      if(pred[col_fill,row_fill] == actu[col_fill,row_fill])
        if(actu[row_fill,col_fill] == TRUE)
          actuT_predT = actuT_predT + 1
        else
          actuF_predF = actuF_predF + 1
...
#E_2_finder(l=0.5)
#E_1_finder(l=0.5)
#cf_mat(sigma2,result(l=0.5))

#the problem till now is that it seems that we need focus a range of lambda to creat ROC curve. 
#Till now I find confustion table cannot help us in ROC,but maybe helpful in the report.
#Below is what I have tried, I set lambda from 0.5 to 1.5, divided into 100 value. but there is problem in the prediction function.
lambda=seq(0.5,1.5,length.out = 100)
result2=function(){
  for (i in lambda){
    pred=as.vector(na.exclude(as.vector(result(1))))
    result(1)
    return(result)
  }
}
predict=function(){
  sigma2=sigma!=0
  diag(sigma2)=NA
  sigma2=as.vector(na.exclude(as.vector(sigma2)))
  return(prediction(result2(),sigma2,lambda))
}
#it seems that our function cannot be used in some general ROC function, we may need to calculate tpr,fpr for different lambda and plot them maybe.I will continous on this on Sunday.
```
