...
```{r BIG N versus Computation Time, False postive and False negative}
set.seed(1)
model_set = 'glasso'
N_big_result_df <- data.frame()
p <- 50
n_seq <- seq(100, 20000, 500)
theta <- make_theta(p, q)
sigma <- inv(theta)
E <- get_true_edge(theta)
...
for (method in model_set){
  ...
  for (n in n_seq ) {
    ...
    cv_conf_matrix['n'] <- n
    cv_conf_matrix['time'] <- end_time - start_time
    N_big_result_df <- rbind(N_big_result_df, cv_conf_matrix)
  }
}
write.csv(N_big_result_df, './iterate_by_n_g_big.csv')
```